...
class MemoryTab(QWidget):
    ...
    def _init_ui(self):
        ...
        mapping_defs = [
            ...
            ('HHDV_TABLE',   'HHDV 실제 테이블명', STUDY_SETTINGS.get('HHDV_TABLE', 'HHDT_DSES_YY')),
        ]
    ...
    def _save_hana_map(self):
        ...
        STUDY_SETTINGS['HHDV_TABLE'] = edits['HHDV_TABLE'].text().strip() or 'HHDT_DSES_YY'
        table_map = STUDY_SETTINGS.get('HANA_TABLE_MAP', {})
...
class DataLoadTab(QWidget):
    ...
    def _init_ui(self):
        ...
        # ── 코호트 ID 추출 설정 (1단계) ──────────────────────────────────────
        cg = QGroupBox("① 코호트 ID 추출 (HHDT_DSES_YY + T20, 진입기간 월별)")
        cl = QGridLayout(cg)
    ...
    def extract_cohort_ids(self):
        """① 코호트 ID 추출 버튼 — HHDT_DSES_YY + T20에서 대상자 ID를 먼저 추출."""
        mw = self.ctx.main_window
